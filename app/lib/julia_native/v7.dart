...
Future<Uint8List> _getSetAsHeightMapParallel(
  ...
  for (var i = 0; i < heightPoints; i += blockSize) {
    futures.add(pool
        .scheduleJob<TransferableTypedData>(GetBlockJob(
            ...
            position: _position))
        .then((v) {
      list.setAll(i * widthPoints, v.materialize().asUint8List());
    }));
  }
...
class GetBlockJob extends PooledJob<TransferableTypedData> {
  ...
  @override
  Future<TransferableTypedData> job() async {
    ...
    return TransferableTypedData.fromList([result]);
  }
}
